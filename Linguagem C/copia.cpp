...
struct cadastro // criou um registro para cadastrar as paradas
{
	...
	char data[200];
	int vazio, cod;
	struct pedido clientePed; // não entendi
	
}log[200];

int verifica_pos(void);  // posição no vetor
...
void consultaCod (void);    // consulta por código
void excluirCliente (void);    // excluir

int main(void){ // inicia a função principal
...
void cadastroPedido(){
	...
	int OpcaoPedido;
	char nomeCad[200];
	printf("\nDigite seu nome como está no cadastro: ");
	gets(nomeCad);
	for(i=0;i<200;i++){
